...
The Aegisum Core daemon can be configured to act as a trusted "border
router", implementing the aegisum wire protocol and relay, making
...
The ZeroMQ feature in Aegisum Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building aegisumd:
...
For instance:
    ...
    $ aegisumd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/aegisumd.tx.raw \
...
These options can also be provided in aegisum.conf.
...
From the perspective of aegisumd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into aegisumd directly. Furthermore, no information is
...
using. Aegisumd appends an up-counting sequence number to each
