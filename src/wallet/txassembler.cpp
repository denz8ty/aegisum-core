    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional AEGS outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes AEGS and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::AEGS_TO_AEGS) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_aegs);
        }
    } else {
        // First try to construct a AEGS-to-AEGS transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::AEGS_ONLY;
        ...
        // If AEGS-to-AEGS fails, create a peg-out transaction
