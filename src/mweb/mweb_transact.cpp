...
TxType MWEB::GetTxType(const std::vector<CRecipient>& recipients, const std::set<CInputCoin>& input_coins)
{
    ...
    static auto is_aegs = [](const CInputCoin& input) { return !input.IsMWEB(); };
    ...
    if (recipients.front().IsMWEB()) {
        ...
        if (std::any_of(input_coins.cbegin(), input_coins.cend(), is_aegs)) {
            return TxType::PEGIN;
        } else {
        ...
        // Otherwise, if there are no MWEB inputs, it's a simple AEGS-to-AEGS transaction.
        if (std::any_of(input_coins.cbegin(), input_coins.cend(), is_mweb)) {
            return TxType::PEGOUT;
        } else {
            return TxType::AEGS_TO_AEGS;
        }
    }
}
...
uint64_t MWEB::CalcMWEBWeight(const MWEB::TxType& mweb_type, const bool change_on_mweb, const std::vector<CRecipient>& recipients)
{
    ...
    if (mweb_type != MWEB::TxType::AEGS_TO_AEGS) {
...
void Transact::AddMWEBTx(InProcessTx& new_tx)
{
    ...
    // Lookup the change paid on the AEGS side
    CAmount aegs_change = 0;
    if (new_tx.change_position != -1) {
        assert(new_tx.tx.vout.size() > (size_t)new_tx.change_position);
        aegs_change = new_tx.tx.vout[new_tx.change_position].nValue;
    }

    // Calculate pegin_amount
    boost::optional<CAmount> pegin_amount = boost::none;
    CAmount aegs_input_amount = std::accumulate(
    ...
    if (aegs_input_amount > 0) {
        const CAmount aegs_fee = new_tx.total_fee - new_tx.mweb_fee;
        assert(aegs_fee <= aegs_input_amount);
        pegin_amount = (aegs_input_amount - (aegs_fee + aegs_change));
    }

    // Add Change
    if (new_tx.change_on_mweb) {
        receivers.push_back(BuildChangeRecipient(new_tx, pegin_amount, aegs_change));
    }
...
mw::Recipient Transact::BuildChangeRecipient(const InProcessTx& new_tx, const boost::optional<CAmount>& pegin_amount, const CAmount& aegs_change)
{
    ...
    CAmount change_amount = (pegin_amount.value_or(0) + mweb_input_amount) - (recipient_amount + new_tx.mweb_fee + aegs_change);
    if (change_amount < 0) {
