...
"""Class for aegisumd node under test"""
...
class TestNode():
    """A class for representing a aegisumd node under test.
    ...
    def __init__(self, i, datadir, *, chain, rpchost, timewait, timeout_factor, bitcoind, bitcoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False):
        ...
        self.bitcoinconf = os.path.join(self.datadir, "aegisum.conf")
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, **kwargs):
        ...
        self.running = True
        self.log.debug("aegisumd started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the aegisumd process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'aegisumd exited with status {} during initialization'.format(self.process.returncode)))
            try:
                rpc = get_rpc_proxy(
        ...
        self._raise_assertion_error("Unable to connect to aegisumd after {}s".format(self.rpc_timeout))
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into aegisumd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to aegisumd
        expected_msg: regex that stderr should match when aegisumd fails

        Will throw if aegisumd starts without an error.
        Will throw if an expected_msg is provided and it does not match aegisumd's stdout."""
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug('aegisumd failed to start: %s', e)
                self.running = False
                self.process = None
                ...
                if expected_msg is None:
                    assert_msg = "aegisumd should have exited with an error"
                else:
                    assert_msg = "aegisumd should have exited with expected error " + expected_msg
...
class TestNodeCLI():
    """Interface to aegisum-cli for an individual node"""
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run aegisum-cli command. Deserializes returned string as python object."""
        pos_args = [arg_to_cli(arg) for arg in args]
        named_args = [str(key) + "=" + arg_to_cli(value) for (key, value) in kwargs.items()]
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same aegisum-cli call"
        ...
        self.log.debug("Running aegisum-cli {}".format(p_args[2:]))
