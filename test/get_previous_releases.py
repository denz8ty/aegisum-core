...
def download_binary(tag, args) -> int:
    if Path(tag).is_dir():
        ...
        shutil.rmtree(tag)
    Path(tag).mkdir()
    release_path = 'aegisum-{}'.format(tag[1:])
    os = 'linux' # TODO
    tarball = 'aegisum-{tag}-{platform}.tar.gz'.format(
        tag=tag[1:], platform=args.platform)
    tarballUrl = 'https://download.aegisum.org/{release_path}/{os}/{tarball}'.format(
    ...
    ret = subprocess.run(['tar', '-zxf', tarball, '-C', tag,
                          '--strip-components=1',
                          'aegisum-{tag}'.format(tag=tag[1:])]).returncode
    if ret:
        return ret
...
def build_release(tag, args) -> int:
    ...
    with pushd(tag):
        ...
        files = ['aegisumd', 'aegisum-cli', 'aegisum-tx']
        for f in files:
